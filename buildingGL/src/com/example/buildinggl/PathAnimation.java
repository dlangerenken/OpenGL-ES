...
public class PathAnimation {
	...
	public PathAnimation(List<Vector3D> pointsOnPath, float timeToFinish,
			boolean shouldRotate, Float length, Float height, Float width,
			int repeatCount) {
	...
	public void animate(float[] modelMatrix) {
		...
		if (!isFinished) {
			...
			Matrix.translateM(modelMatrix, 0, currentPosition.getX(),
					currentPosition.getY(), currentPosition.getZ());

			if (shouldRotate) {
				GLHelper.rotateModel(modelMatrix, yaw, pitch, null, true,
						width, length, height);
			}
		}
	}
